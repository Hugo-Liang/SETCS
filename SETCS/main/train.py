...
import SETCS.config as config
import SETCS.inputters.utils as util
from SETCS.inputters import constants

from collections import OrderedDict, Counter
from tqdm import tqdm
from SETCS.inputters.timer import AverageMeter, Timer
import SETCS.inputters.vector as vector
import SETCS.inputters.dataset as data

from SETCS.main.model import Code2NaturalLanguage
from SETCS.eval.bleu import corpus_bleu
from SETCS.eval.rouge import Rouge
# from SETCS.eval.meteor import Meteor
...
def add_train_args(parser):
    ...
    files.add_argument('--train_src_tokenized', nargs='+', type=str,
                       help='Tokenized train source file')
    ...
    files.add_argument('--dev_src_tokenized', nargs='+', type=str,
                       help='Tokenized dev source file')
...
def set_defaults(args):
    ...
    if not args.only_test:
        ...
        args.train_src_tokenized_files = []
        ...
        if num_dataset > 1:
            ...
            if len(args.train_src_tokenized) == 1:
                args.train_src_tokenized = args.train_src_tokenized * num_dataset
        ...
        for i in range(num_dataset):
            ...
            if args.use_tokenized_code:
                train_src_tokenized = os.path.join(data_dir, args.train_src_tokenized[i])
                if not os.path.isfile(train_src_tokenized):
                    raise IOError('No such file: %s' % train_src_tokenized)
            else:
                train_src_tokenized = None
            ...
            args.train_src_tokenized_files.append(train_src_tokenized)
            args.train_tgt_files.append(train_tgt)
            args.train_src_tag_files.append(train_src_tag)

    args.dev_src_files = []
    args.dev_src_tokenized_files = []
    args.dev_tgt_files = []
    ...
    for i in range(num_dataset):
        ...
        if args.use_tokenized_code:
            dev_src_tokenized = os.path.join(data_dir, args.dev_src_tokenized[i])
            if not os.path.isfile(dev_src_tokenized):
                raise IOError('No such file: %s' % dev_src_tokenized)
        else:
            dev_src_tokenized = None
        if args.use_code_type:
        ...
        args.dev_src_tokenized_files.append(dev_src_tokenized)
...
def validate_official(args, data_loader, model, global_stats, mode='dev'):
    ...
    #                                                                mode=mode)
    bleu, rouge_l, precision, recall, f1 = eval_accuracies(hypotheses,
                                                                   references,
                                                                   copy_dict,
                                                                   sources=sources,
                                                                   filename=args.pred_file,
...
def eval_accuracies(hypotheses, references, copy_info, sources=None,
    ...
    return bleu * 100, rouge_l * 100, precision.avg * 100, \
           recall.avg * 100, f1.avg * 100
...
def main(args):
    ...
    if not args.only_test:
        ...
        for train_src, train_src_tokenized, train_src_tag, train_tgt, dataset_name in \
                zip(args.train_src_files, args.train_src_tokenized_files, args.train_src_tag_files,
                    args.train_tgt_files, args.dataset_name):
            train_files = dict()
            train_files['src'] = train_src
            train_files['src_tokenized'] = train_src_tokenized
    ...
    dev_exs = []
    for dev_src, dev_src_tokenized, dev_src_tag, dev_tgt, dataset_name in \
            zip(args.dev_src_files, args.dev_src_tokenized_files, args.dev_src_tag_files,
                args.dev_tgt_files, args.dataset_name):
        dev_files = dict()
        dev_files['src'] = dev_src
        dev_files['src_tokenized'] = dev_src_tokenized
