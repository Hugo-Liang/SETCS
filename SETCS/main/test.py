...
from SETCS.main.train import compute_eval_score
...
from SETCS.translator.translator import Translator
from SETCS.translator.beam import GNMTGlobalScorer
from SETCS.translator.translation import TranslationBuilder
...
def add_test_args(parser):
    ...
    files.add_argument('--model_dir', type=str, default='/tmp/qa_models/',
                       help='Directory for saved model/checkpoints/logs')
    ...
    bsearch.add_argument('--replace_unk', default=True,
...
def set_defaults(args):
    ...
    if num_dataset > 1:
        ...
        if len(args.dev_src_tokenized) == 1:
            args.dev_src_tokenized = args.dev_src_tokenized * num_dataset
...
def prepare_batch(batch, model):
    ...
    code_word_tokenized_rep = batch['code_word_tokenized_rep']
    ...
    batch_inputs['code_word_tokenized_rep'] = code_word_tokenized_rep
...
def validate_official(args, data_loader, model):
    ...
    if args.only_generate:
        ...
        bleu, rouge_l, precision, recall, f1, ind_bleu, ind_rouge = \
            eval_accuracies(hypotheses, references)
