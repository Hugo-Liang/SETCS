...
def vectorize(ex, model):
    ...
    vectorized_ex['code_tokenized_tokens'] = code.tokenized_tokens
    ...
    if model.args.use_tokenized_code:
        vectorized_ex['code_word_tokenized_rep'] = torch.LongTensor(code.vectorize(word_dict=src_dict))
    ...
    vectorized_ex['use_tokenized'] = model.args.use_tokenized_code
...
def batchify(batch):
    ...
    use_tokenized = batch[0]['use_tokenized']
    ...
    if use_tokenized:
        code_words_tokenized = [ex['code_word_tokenized_rep'] for ex in batch]
    ...
    code_word_tokenized_rep = torch.zeros(batch_size, max_code_len, dtype=torch.long) \
        if use_tokenized else None
    ...
    for i in range(batch_size):
        ...
        if use_tokenized:
            code_word_tokenized_rep[i, :code_words_tokenized[i].size(0)].copy_(code_words_tokenized[i])
        if use_code_type:
